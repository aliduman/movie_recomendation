...
export default function ExplorePage() {
  ...
  const { movies, loading, loadingMore, hasMore, fetchByGenre, searchMovies, loadMore } = useMovies();
  ...
  return (
    ...
    >
      <div className="text-center mb-8">
        <h1 className="text-3xl sm:text-4xl font-extrabold mb-2">🎯 {t('explore.title')}</h1>
        <p className="text-gray-400">{t('explore.subtitle')}</p>
      </div>

      <div className="space-y-6 mb-10">
        <SearchBar onSearch={handleSearch} />
        {!isSearching && <GenreSelector selected={selectedGenre} onSelect={handleGenre} />}
      </div>
