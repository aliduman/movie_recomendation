...
export default function WatchlistPage() {
  ...
  const availableGenres = useMemo(() => {
    const ids = new Set(watchlist.flatMap((m) => m.genre_ids || []));
    return GENRES.filter((g) => ids.has(g.id));
  }, [watchlist]);

  const filtered = useMemo(() => {
    if (!selectedGenre) return watchlist;
    return watchlist.filter((m) => m.genre_ids?.includes(selectedGenre));
    ...
    >
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-gray-500 text-lg">{t('watchlist.empty')}</p>
          <p className="text-gray-600 text-sm mt-1">{t('watchlist.emptyHint')}</p>
        </motion.div>
      ) : (
        <>
          {availableGenres.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              >
                🎬 {t('watchlist.all')} ({watchlist.length})
              </button>

              {availableGenres.map((genre) => {
                const count = watchlist.filter((m) => m.genre_ids?.includes(genre.id)).length;
